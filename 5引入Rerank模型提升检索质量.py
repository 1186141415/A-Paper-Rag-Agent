...
# ===== rerank =====
rank_result = rerank(question, relevant_chunks)
print("Rerank result:", rank_result)

import ast

try:
    sorted_indices = ast.literal_eval(rank_result)
except:
    print("Rerank parse failed, fallback to original order")
    sorted_indices = list(range(len(relevant_chunks)))

top_k = 3  # 可以调：2 或 3

best_chunks = [relevant_chunks[i] for i in sorted_indices[:top_k]]
